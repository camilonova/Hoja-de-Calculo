...
public class DialogoInsertar extends JDialog implements KeyListener {
	 ...
	 * si se quieren insertar filas o columnas y cuantas.<p>
			...
			public void actionPerformed(ActionEvent e) {
				try {
					if(filasRadioBtn.isSelected() && Integer.parseInt(cantidadFilas.getText()) > 0) {
						((PanelHojaCalculo)HojaCalculo.getInstancia().constructor).insertarFilas(Integer.parseInt(cantidadFilas.getText()));
					}
					else if(columnasRadioBtn.isSelected() && Integer.parseInt(cantidadColumnas.getText()) > 0) {
						((PanelHojaCalculo)HojaCalculo.getInstancia().constructor).insertarColumnas(Integer.parseInt(cantidadColumnas.getText()));
					}
					else
